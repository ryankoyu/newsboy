...
## ⚠️ 이 문서는 더 이상 "기본 화면"이 아니다 (2026-08-11)

이 문서가 규정하는 화면(☕ BRIEFLY 헤더 · 인사말 블록 · 카테고리 요약 칩 등, 이하 **표준 스킨**)은 **현재 기본으로 뜨는 화면이 아니다.**

`web/src/components/newsprint/useNewsprintSkin.ts`의 `useNewsprintSkin()`(선언 `export function useNewsprintSkin(): boolean {` `:19`, 판정 `const isDark = hydrated && (theme === "dark" || (theme === "system" && systemDark));` `:23`, 반환 `return !isDark;` `:24` — 줄 번호 2026-08-12 **7차** 재실측. 4차가 세 문서에 함께 써넣은 `:20`/`:23`/`:25`는 선언과 반환이 한 칸씩 밀려 있었다)은 **다크 모드가 아닌 모든 경우 `true`** 를 반환하고, 그때 `Skinned*View` 컴포넌트들이 **뉴스프린트(1902년 신문) 스킨**을 렌더한다(`NewsprintFrontPage` 모바일 / `NewsprintBroadsheet` 데스크톱). 커밋 `c95660e`.

- **기본 = 뉴스프린트 스킨** (라이트 전용 — 종이를 반전시키지 않는다는 설계 경계)
- **표준 스킨(이 문서) = 다크 모드 폴백**

이 문서의 §2 화면 스펙·와이어프레임은 그 폴백 경로에 대해서는 여전히 유효하다. 하지만 **뉴스프린트 스킨의 설계 근거·타이포·그리드·컷 이미지 규칙은 아직 이 문서에 흡수되지 않았다** — 지금은 핸드오프 문서와 `docs/newsprint-cuts/`에만 있다. 이 미흡수 상태 자체가 `docs/feature-status.md` §2 **G13**으로 등록돼 있다.

다음에 이 문서를 손보는 사람이 할 일: 뉴스프린트 스킨 절을 신설하고, 두 스킨 중 어느 쪽이 기준인지 §0 디자인 토큰 수준에서 정리할 것. **그 전까지는 "구현 담당이 그대로 코딩할 수 있는 스펙"이라는 이 문서의 전제가 절반만 참이다.**
...
> ⚠️ 아래 와이어프레임은 **다크 모드에서만** 실제로 뜬다. 라이트/시스템(라이트)에서는 뉴스프린트 스킨의 `NewsprintFrontPage`(모바일) / `NewsprintBroadsheet`(데스크톱)가 대신 렌더된다 — 문서 상단 경고 참조.

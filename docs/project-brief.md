...
> **이 흐름도는 구상이며 현재 구현 상태가 아니다.** 실제로 무엇이 동작하는지는 `docs/feature-status.md` 가 단일 기준이다. 이 흐름도 기준으로 짚으면: Vocabulary 는 구현됨, Quiz 는 2026-07-10 사용자 지시로 MVP 제외(보류), Shadowing·AI Conversation 은 V1.5~V2 보류다.
>
> **정정(2026-08-12) — "Grammar 포인트" 삭제.** 이전 판의 이 흐름도는 Vocabulary 다음에 "Grammar 포인트"를 산출물로 적었다. 실측하면 문법 산출물은 어디에도 없다 — `pipeline/src`·`web/src` 전체에 `grammar`/`문법` 문자열이 0건이고(프롬프트·타입·DB 스키마·웹 타입 어디에도 필드가 없다), `feature-status.md` 의 §1 인벤토리에도 §3 의도적 보류 목록에도 등재된 적이 없다 `[관찰]`. 즉 구현된 적도, 미루기로 정해진 적도 없이 이 흐름도에만 남아 있던 항목이라, 흐름도에서 뺀다. 문법을 다시 하기로 한다면 그때 §16 목록과 `feature-status.md` 에 정식으로 올린다.
>
> 참고: §9 의 **AI Tutor / Conversation** 은 답변에 대한 문법 교정을 포함하지만, 그것은 대화 기능의 일부이고 위 흐름도가 말하던 "기사마다 문법 포인트를 뽑아 붙이는 산출물"과는 다른 것이다.
...
- **원문 기사의 사진·삽화는 사용하지 않는다.** 기사 제목 그대로 사용도 피한다.
  - 직접 만든 이미지(스톡 이미지 또는 AI 일러스트)는 허용한다 — 단 `docs/news-sourcing-strategy.md` §3의 조건대로 **보도사진이 아니라는 고지**를 함께 붙인다. *(2026-08-12 정정: 이 줄은 원래 "기사 사진·삽화는 사용하지 않는다"였는데, 그대로 읽으면 지금 배포되는 화면을 금지한다. 뉴스프린트 스킨은 하루 lead 기사 1건에 자체 제작 컷을 렌더하고 `newsprint/chrome.tsx`의 `ILLUSTRATION_NOTICE`로 고지까지 함께 낸다. §14는 저작권 절이므로 원래 뜻은 "원문의" 사진·삽화였고 그 한 단어가 빠져 있었다.)*
